...
function DropDown({
  ...
  return (
    <ClickAwayListener onClickAway={() => setAnchorEL(null)}>
      <div
        className={clsx(
          "dropdown W(100%) Fx(1) D(f) Fxd(c) Gap(1rem)",
          className
        )}
        style={style}
      >
        ...
        <ButtonRipple
          onClick={handleOpen}
          className={clsx(
            "H(4rem) Bd(border) Bdrs(4rem) Px(2rem)  Fz(16px) W(100%) D(f) Ai(c) Jc(sb)",
            btnClass
          )}
        >
        ...
        >
          <div className="Miw(12rem)  Bdrs(1rem) Ov(h) Bgc(#fff) D(f) Fxd(c) Jc(fs)">
            {options.map((item, i) => (
              <ButtonRipple
                className={clsx(
                  "D(f) Ai(c) Px(2rem) H(4rem) hover Bd(n)",
                  active === item && "Bgc(lightgray) Op(.8)"
                )}
                key={item}
                ref={(ref: HTMLButtonElement) =>
                  handleCurrentItemRef(ref, item)
                }
