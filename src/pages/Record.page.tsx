...
import { createEntry } from "../lib/database/create.db";
...
function Entries() {
  const { id } = useParams();
  const entries = useRecoilValue(UserRecordState(id));
  const total = entries.reduce(
    (pre, { action, value }) =>
      action === ActionType.PLUS ? pre - value : pre + value,
    0
  );
  return (
